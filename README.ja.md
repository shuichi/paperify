...
--browser-executable <file>
                      Mermaid と PDF 出力に使う Chrome/Chromium 実行ファイル
--watch               ファイル変更時に再ビルドする
--copy-assets         互換性のためのオプション。画像や poster はコンパイル時にインライン化される
--help                ヘルプを表示する
```

Paperify は Markdown を自己完結した HTML ファイルにコンパイルします。コンパイル済み HTML には Paperify CSS、ローカル画像、Mermaid SVG、動画 poster/fallback、KaTeX CSS、KaTeX フォントが data URI として埋め込まれるため、HTML 単体で開けます。動画ファイル本体は埋め込まれません。ローカル動画ソースを再生用に出力先の隣へコピーしたい場合は `--copy-assets` を使ってください。
...
このリポジトリには、同じ機能を試せる英語と日本語のサンプル論文が含まれています。frontmatter、数式、引用、図、Mermaid 図、表、コード、脚注、動画を確認できます。
...
`mermaid` タグ付きの fenced code block は、ビルド時に SVG へレンダリングされます。
...
SVG はセマンティックな figure 内の画像 data URI として埋め込まれます。完成した HTML には Mermaid のソースや実行時 JavaScript は残らず、CDN にも接続しません。Mermaid の `accDescr` は画像の `alt` に、`accTitle` は `title` になります。どちらもなければ、アクセシビリティ用の fallback として `Mermaid diagram` を使います。

Mermaid のレンダリングに Chromium を使うのはビルド時だけです。不正な図は、CLI と PDF ビルドでは Mermaid parser のエラーとして失敗します。VS Code のライブプレビューでは、編集中のソースコードブロックを残し、警告として報告します。
...
src/
  ...
  mermaid.ts                 静的 SVG 用の注入可能な Chromium renderer
